...
export const registerUser=asyncHandler( async(req,res)=>{
   //get user details from frontend
    ...
    if (
        ...
        res.status(400).json({ error:"Enter the all required fields"})
    ...
    if (!avatarLocalPath) {
        res.status(404).json({ error:"avatar file has to be upload"})    
    ...
    if (!avatar) {
        res.status(404).json({ error:"Avatar file is required"});
